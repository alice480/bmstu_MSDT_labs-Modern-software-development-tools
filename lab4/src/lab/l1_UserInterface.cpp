...
void performCommandsSimultaneously(std::istream &is, ItemCollector &col,
                                   const TerminalOutput &out,
  ...
  std::cerr << "Выполняем пакет команд. Размер пула потоков: " << tp.size()
            << ", остаток команд в очереди: " << tp.queue_length() << " из "
            << number_of_commands << std::endl;
}
...
void print_instruction() {
  ...
  std::cout << "aI/addImage [museum_number] [author_name] [image_name] "
               "[year_of_creation] [exhibited]"
            << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  // Работа с файлом команд через файл, а не пайп может быть полезна, если нужна
  // отладка
